...
@click.option("--rollup", is_flag=True, help="Summarize all chapters and roll up into a master summary")
def summarize(
    ...
    if interactive_mode:
        from audiobench.cli.wizard import prompt_transcription, prompt_menu
        try:
            ...
            if not custom_prompt:
                custom_prompt = prompt_menu(
                    ...
                    [
                        ...
                        ("Action Items", "Extract action items and next steps", "Focus on action items"),
                        ("Bullet Points", "Key takeaways in bullet points", "Extract key takeaways as bullet points"),
                    ]
                )
    ...
    if chapter is not None:
        if not audio_file_id:
            console.print(error_panel("No audio file linked", "Cannot find chapters without an audio_file_id."))
            sys.exit(1)
        chap = chap_repo.get_chapter_by_index(audio_file_id, chapter)
        if not chap or not chap["transcription_id"]:
            console.print(error_panel("Chapter Not Ready", f"Chapter {chapter} is either missing or not transcribed yet."))
            sys.exit(1)
        ...
        summary_text = _generate_summary(client, chap_record, custom_prompt, model_name, title_suffix=f" (Chapter {chapter})")
        if summary_text:
    ...
    if all_chapters or rollup:
        ...
        for i, chap in enumerate(chapters, start=1):
            ...
            if chap_record:
                ...
                summary_text = _generate_summary(client, chap_record, custom_prompt, model_name, title_suffix=f" (Chapter {i})")
                if summary_text:
        ...
        if rollup and chapter_summaries:
            console.print(f"  [{BOLD} {ACCENT}]Rolling up {len(chapter_summaries)} chapter summaries into a Master Summary...[/]")
            ...
            # Save master report
            _save_report(settings, record, summary_text, suffix="_master", extra_content=master_prompt)
...
def _save_report(settings, record, summary_text: str, suffix: str = "", extra_content: str = "") -> None:
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        if suffix == "_master":
            f.write(f"# Master Summary: {record['file_name']}\n\n{summary_text}\n\n---\n\n{extra_content}")
        else:
