...
| `MemoryStore` | `memory/memory_store.py` | LanceDB vector memory store with atomic `merge_insert()` & scalar index |
| `DaemonServer` | `daemon/server.py` | Single-instance UNIX socket daemon & background process runner |
| `SweepState` | `daemon/sweep_state.py` | Thread-safe daemon state & segment sub-batch diagnostic tracking |
| `JobRunner` | `jobs/runner.py` | Asynchronous SQLite job queue execution worker |
| `DriftDetector` | `daemon/intelligence/drift_detector.py` | Background intelligence loop for audio/transcript alignment drift |
...
AudioBench uses a dual-engine storage architecture:
1. **SQLite (Relational Metadata & History)**: Managed via SQLAlchemy + Alembic-style migrations (`storage/migrations/`). Stores core metadata, session history, job queues, and bookmark annotations.
2. **LanceDB (Vector Memory & RAG)**: Embedded vector database (`memory/memory_store.py`) storing embedded transcript expressions for hybrid vector retrieval.

### SQLite Database Tables

| Table | Key Columns | Purpose |
|-------|-------------|---------|
| `audio_files` | `id`, `file_hash` (SHA-256), `file_name`, `file_path`, `duration_seconds` | Core audio file catalog |
| `transcriptions` | `id`, `audio_file_id` (FK), `full_text`, `language`, `model_name`, `engine`, `created_at` | Transcript metadata |
| `segments` | `id`, `transcription_id` (FK), `text`, `start`, `end`, `speaker` | Time-stamped transcript segments |
| `bookmarks` | `id`, `audio_file_id` (FK), `timestamp`, `end_timestamp`, `name`, `type`, `notes` | User & AI-generated annotations |
| `chat_sessions` | `id`, `title`, `model`, `created_at` | Interactive chat threads |
| `chat_messages` | `id`, `session_id` (FK), `role`, `content`, `thinking` | LLM conversation history |
| `job_queue` | `id`, `task_type`, `payload`, `status`, `created_at`, `updated_at` | Unified background job queue |
| `works` | `id`, `title`, `author`, `privacy_tier`, `created_at` | Study works & projects |

### LanceDB Vector Store (`expressions` Table)

Stores 768-dimensional normalized text embeddings generated by `nomic-embed-text-v1.5`.

| Column | Type | Description |
|--------|------|-------------|
| `expression_id` | `int64` (Indexed) | Primary key (indexed via scalar index for fast `merge_insert` upserts) |
| `content` | `string` | Text content of the expression |
| `source_type` | `string` | Source origin (`audio_transcript`, `note`, `imported_work`) |
| `speaker` | `string` (Nullable) | Speaker label |
